%EERX crossover operator
function NewChrom = EERX_crossover(OldChrom, XOVR)
...
for row=1:2:nbRows
    ...
    if rand<XOVR  % recombine with a given probability
        NewChrom(row,:) = enhanced_edge_recombination_crossover(OldChrom(row:row+1,:));
        NewChrom(row+1,:)= enhanced_edge_recombination_crossover(OldChrom(row+1:-1:row,:));
    end
end
end
% End of function
